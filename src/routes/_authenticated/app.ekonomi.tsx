...
export const Route = createFileRoute("/_authenticated/app/ekonomi")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Se din avgift eller hyra, förfallodatum och betalningshistorik." },
...
function EconomyPage() {
  ...
  const paidThisYear = payments
    .filter((p) => p.status === "paid" && new Date(p.period).getFullYear() === new Date().getFullYear())
  ...
  return (
    <div>
      <PageHeader
        title="Ekonomi"
        subtitle={isRent ? "Din hyra och betalningshistorik" : "Din månadsavgift och betalningshistorik"}
      />
